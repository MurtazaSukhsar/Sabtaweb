...
const CARD_SIZE = 144
const LINE_SPACING = CARD_SIZE + 30 // px gap between cards when lined up mid-animation
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className="absolute inset-0 flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-xl border border-primary bg-primary p-4 text-center shadow-lg"
            style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
          >
            <p className="line-clamp-2 text-[13px] font-bold leading-tight text-primary-foreground">{product.name}</p>
            {(product.grade || product.standard) && (
              <p className="mt-1 line-clamp-1 text-[10px] font-semibold text-primary-foreground/70">
                {product.grade}
                {product.grade && product.standard && " · "}
                {product.standard}
              </p>
            )}
            <p className="mt-1.5 text-[10px] font-bold uppercase tracking-widest text-accent">View details</p>
          </div>
        </motion.div>
      </Link>
    </motion.div>
    ...
    <section ref={wrapperRef} className="relative h-[220vh] sm:h-[240vh]">
      <div className="sticky top-0 flex h-screen min-h-[640px] w-full flex-col justify-center overflow-hidden py-10">
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 md:px-8 lg:px-12">
          ...
          <div
            ref={stageRef}
            className="relative flex h-[64vh] min-h-[480px] w-full items-center justify-center overflow-hidden"
          >
            {items.map((product, i) => {
              ...
              if (progress < SCATTER_END) {
                ...
                // Circle formation.
                const circleRadius = Math.min(minDimension * 0.34, 280)
                ...
                const arcWidth = Math.min(containerSize.width * 0.9, isMobile ? 380 : 1080)
                const baseYOffset = containerSize.height * (isMobile ? 0.08 : 0.06)
                const bowDepth = containerSize.height * (isMobile ? 0.2 : 0.24)
