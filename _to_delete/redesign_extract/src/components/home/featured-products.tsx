    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className="absolute inset-0 flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-xl border border-primary bg-primary p-2 text-center shadow-lg"
            style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
          >
            <p className="line-clamp-2 text-[9px] font-bold leading-tight text-primary-foreground">{product.name}</p>
            {(product.grade || product.standard) && (
              <p className="mt-1 line-clamp-1 text-[7px] font-semibold text-primary-foreground/70">
                {product.grade}
                {product.grade && product.standard && " · "}
                {product.standard}
              </p>
            )}
            <p className="mt-1 text-[7px] font-bold uppercase tracking-widest text-accent">View details</p>
          </div>
        </motion.div>
      </Link>
    </motion.div>
        ...
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 md:px-8 lg:px-12">
          <ScrollReveal>
            <div className="mb-8 flex flex-col items-start gap-3 md:mb-10">
              ...
              <h2 className="section-heading">Products our customers reach for most</h2>
              <p className="section-subheading">
                Keep scrolling: the fasteners and hardware we stock, certify, and ship first fall into place as you
                go. Hover any piece for details, or click through to the full spec sheet.
              </p>
            </div>
          </ScrollReveal>
          ...
          <div
            ref={stageRef}
            className="relative flex h-[52vh] min-h-[360px] w-full items-center justify-center overflow-hidden rounded-2xl border border-border bg-white"
          >
            {items.map((product, i) => {
              ...
              if (progress < SCATTER_END) {
                ...
                // Circle formation.
                const circleRadius = Math.min(minDimension * 0.32, 230)
                ...
                const arcWidth = Math.min(containerSize.width * 0.84, isMobile ? 300 : 940)
