...
export function ImageStreamHero({
  ...
  const css = React.useMemo(
    () =>
      ...
      // finished still instead of collapsing onto the axis. The duration and
      // iteration-count are reasserted with !important too: this site has a
      // sitewide prefers-reduced-motion reset (animation-duration: 1ms,
      // iteration-count: 1, both !important) for every other animation on the
      // page. Left alone, that reset would win the cascade over our own
      // 18s/infinite values, so each card's large negative delay would run
      // straight past its now-1ms single iteration and fall back to the
      // unanimated base box — every card landing on the same untransformed
      // spot, which is precisely the collapsed-axis hole this design avoids.
      `@media(prefers-reduced-motion:reduce){.${card}{animation-duration:${speed}s!important;animation-iteration-count:infinite!important;animation-play-state:paused!important}}`,
